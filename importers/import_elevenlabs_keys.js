...
const config = {
  csv: args.get('csv') || process.env.OMNIROUTE_ELEVENLABS_CSV || 'elevenlabs.csv',
...
async function main() {
  console.log('=== ElevenLabs OmniRoute Connection Automation ===');
  console.log(`Reading keys from: ${config.csv}`);
  const rows = readKeys(config.csv);
  
  // Filter for unique ElevenLabs keys
  const seenKeys = new Set();
  const uniqueRows = [];
  for (const row of rows) {
    // Only import keys starting with sk_
    if (!row.apiKey.startsWith('sk_')) continue;
    if (seenKeys.has(row.apiKey)) continue;
    seenKeys.add(row.apiKey);
    uniqueRows.push(row);
  }

  console.log(`Found ${rows.length} total rows. Unique ElevenLabs keys: ${uniqueRows.length}`);
  if (uniqueRows.length === 0) {
    console.log('No ElevenLabs keys to import.');
    return;
  }
  ...
  try {
    ...
    const existingNames = new Set(allConnections
      .filter(connection => connection.provider === 'elevenlabs')
