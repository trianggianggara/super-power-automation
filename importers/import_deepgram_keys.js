...
const config = {
  csv: args.get('csv') || process.env.OMNIROUTE_DEEPGRAM_CSV || 'deepgram.csv',
...
async function validateKey(apiKey) {
  // Simple check using deepgram endpoint if validation is needed
  try {
...
async function main() {
  console.log('=== Deepgram OmniRoute Connection Automation ===');
  ...
  console.log(`Found ${rows.length} total rows. Unique keys: ${uniqueRows.length}`);
  ...
  try {
    ...
    const existingNames = new Set(allConnections
      .filter(connection => connection.provider === 'deepgram')
