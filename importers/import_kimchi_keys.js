...
const config = {
  csv: args.get('csv') || process.env.OMNIROUTE_KIMCHI_CSV || 'kimchi.csv',
...
function readKeys(csvPath) {
  ...
  return lines.slice(1).map(line => Object.fromEntries(parseCsvLine(line).map((value, index) => [header[index], value])))
    .map(row => ({
      name: row.email,
      apiKey: row.api_key || row.apiKey || row.api_key_value || '',
      email: row.email,
      password: row.password,
    }))
    .filter(row => row.name && row.apiKey && row.apiKey !== 'NOT_FOUND');
}

async function validateKey(apiKey) {
  try {
    const res = await fetch('https://api.kimchi.dev/v1/models', {
...
async function main() {
  console.log('=== Kimchi OmniRoute Connection Automation ===');
  console.log(`Reading keys from: ${config.csv}`);
  const rows = readKeys(config.csv);
  
  // Filter for unique Kimchi keys
  ...
  console.log(`Found ${rows.length} total rows. Unique Kimchi keys: ${uniqueRows.length}`);
  if (uniqueRows.length === 0) {
    console.log('No Kimchi keys to import.');
    return;
  }
  ...
  try {
    ...
    const existingNames = new Set(allConnections
      .filter(connection => connection.provider === 'kimchi')
    ...
    for (const row of uniqueRows) {
      ...
      if (config.validate) {
        ...
        if (!validation.ok) {
          console.log(`  Validation failed: ${validation.error}. Skipping.`);
