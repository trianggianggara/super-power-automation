...
const config = {
  csv: args.get('csv') || process.env.OMNIROUTE_MISTRAL_CSV || 'mistral.csv',
...
async function main() {
  console.log('=== Mistral OmniRoute Connection Automation ===');
  console.log(`Reading keys from: ${config.csv}`);
  const rows = readKeys(config.csv);
  
  // Filter for unique Mistral keys
  ...
  console.log(`Found ${rows.length} total rows. Unique Mistral keys: ${uniqueRows.length}`);
  if (uniqueRows.length === 0) {
    console.log('No Mistral keys to import.');
    return;
  }
  ...
  try {
    ...
    const existingNames = new Set(allConnections
      .filter(connection => connection.provider === 'mistral')
