...
const DEFAULT_PROVIDER_ID = 'openai-compatible-chat-a045880d-a57c-49b2-be29-6b96c2510998';
...
const config = {
  csv: args.get('csv') || process.env.OMNIROUTE_BASTEN_CSV || 'basten.csv',
  ...
  providerName: args.get('provider-name') || process.env.OMNIROUTE_PROVIDER_NAME || 'baseten',
  baseUrl: args.get('base-url') || process.env.OMNIROUTE_PROVIDER_BASE_URL || 'https://inference.baseten.co/v1',
  prefix: args.get('prefix') || process.env.OMNIROUTE_PROVIDER_PREFIX || 'baseten',
...
function readKeys(csvPath) {
  ...
  return lines.slice(1).map(line => Object.fromEntries(parseCsvLine(line).map((value, index) => [header[index], value])))
    .map(row => ({
      name: row.email,
      apiKey: row.api_key || row.apiKey || row.api_key_name,
      email: row.email,
    }))
...
async function main() {
  console.log('=== Baseten OmniRoute Connection Automation ===');
  ...
  console.log(`Found ${rows.length} total rows. Unique Baseten keys: ${uniqueRows.length}`);
  if (uniqueRows.length === 0) {
    console.log('No Baseten keys to import.');
    return;
  }
