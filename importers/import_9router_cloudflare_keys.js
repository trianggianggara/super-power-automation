...
const config = {
  csv: args.get('csv') || process.env.NINE_ROUTER_CLOUDFLARE_CSV || 'cloudflare.csv',
...
async function login9Router(page) {
  console.log(`Navigating to 9router Cloudflare-AI page: ${config.url}/dashboard/providers/cloudflare-ai`);
  try {
    await page.goto(`${config.url}/dashboard/providers/cloudflare-ai`, { waitUntil: 'domcontentloaded', timeout: 20000 });
  } catch (err) {
  ...
  console.log('  Filling 9router password...');
...
async function main() {
  ...
  try {
    ...
    for (const row of uniqueRows) {
      ...
      console.log(`Saving connection for ${row.name} to 9router${accountId ? ` (Account ID: ${accountId})` : ''}...`);
